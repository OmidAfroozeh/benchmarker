...
def __get_tpcds_file_path(sf: int) -> str:
    file_name =  os.path.join('varchars_variable_length', f'varchars-length-{sf}.db')
...
def __generate_string_microbenchmark_data(str_len: List[int]):
    for (index, sl) in enumerate(str_len):
        ...
        query_tpcds = f"""
            CREATE TABLE varchars AS SELECT concat('{temp_str}', i//10000) AS str1, concat('{temp_str}', i//10000) AS str2 FROM range(10_000_000) tbl(i);
        """
        con.sql(query_tpcds)
        con.close()
