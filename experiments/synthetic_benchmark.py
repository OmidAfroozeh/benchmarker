###############################################################################
# Unified String‑Benchmark Generator & Micro‑Benchmark Helper                 #
###############################################################################
"""string_benchmark.py ─ All‑in‑one utility to create string benchmarks
and assemble micro‑benchmark descriptors, fully parameterised from a driver
script.

The module merges the former *dataset generator* and *variable‑length helper*
into a single public API:

* ``ColumnSpec`` – describes one varchar column.
* ``generate_string_benchmark`` – writes Parquet + DuckDB for a single table.
* ``get_string_len_benchmark`` – convenience factory to create multiple data‑
  sets at various string lengths and return a *Benchmark* descriptor.

Every knob from the original scripts is now an explicit function argument, so
**nothing is hard‑wired**.  Import it in your driver script and pass whatever
you like.
"""
...
# ---------------------------------------------------------------------------
# Fallback stubs so the module also works outside the original repo structure
# ---------------------------------------------------------------------------
try:
    from src.logger import get_logger  # type: ignore
except Exception:  # pragma: no cover – only used when repo modules missing
...
try:
    from src.models import DataSet, Benchmark, Query  # type: ignore
