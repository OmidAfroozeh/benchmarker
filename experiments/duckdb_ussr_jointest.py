    ...
    *,
    ...
    if not parquet_dir.exists():
        ...
        for c in range(n_chunks):
            ...
            df = pd.DataFrame({s.name: s.slice_chunk(total_rows, start, end) for s in column_specs}, copy=False)
            pq.write_table(
    ...
    table_present = bool(con.execute(
    ...
    if not table_present:
        cols = ", ".join(s.name for s in column_specs)
        con.execute(
...
# Systems & runner -----------------------------------------------------------
try:
    from config.systems.duckdb import DUCK_DB_MAIN, UnifiedStringDictionary_lock_free_16mB, UnifiedStringDictionary_lock_free_512K # type: ignore
    from src.runner.experiment_runner import run  # type: ignore
except Exception:  # fallback dummy runner
    DUCK_DB_MAIN = UnifiedStringDictionary_lock_free_16mB = object()

    def run(cfg):  # type: ignore
        logger.warning("Experiment runner missing – dry‑run only.\nWould have executed config:\n%s", cfg)
...
LENGTH_SPECS:    Sequence[LengthSpec] = [128]
...
CUSTOM_QUERIES: List[Query] = [
    {
        "name": "single_column_groupby_a",
        "index": 0,
        "run_script": {"duckdb": f"SELECT * FROM {TABLE_A} CROSS JOIN {TABLE_B}"},
    },
]
...
def assemble_datasets(root: Optional[Path] = None) -> List[DataSet]:
    ...
    for idx, (len_spec, rows, uniques, s_val) in enumerate(
        ...
        build_db_with_multiple_tables(
            duckdb_path=db_path,
            variants=[(TABLE_A, make_column_specs(len_spec, uniques, s_val)),
        ...
        datasets.append({
            ...
            "setup_script": {"duckdb": f"ATTACH '{db_path}' (READ_ONLY); USE '{db_path.stem}';"},
            "config": {"string_length": len_spec, "n_unique": uniques, "zipf_s": s_val, "tables": [TABLE_A, TABLE_B]},
        })
    return datasets
...
def build_benchmark(root: Optional[Path] = None) -> Benchmark:
    return {"name": "string_benchmark_zipf_grid_two_tables", "datasets": assemble_datasets(root), "queries": CUSTOM_QUERIES}


# Runtime settings -----------------------------------------------------------
RUN_SETTINGS    = {"n_parallel": 1, "n_runs": 6}
SYSTEM_SETTINGS = [{"n_threads": 1}]
SYSTEMS         = [DUCK_DB_MAIN, UnifiedStringDictionary_lock_free_512K]
...
def main() -> None:
    ...
    p = argparse.ArgumentParser(description="Generate two‑table varchar benchmarks and optionally run them")
    p.add_argument("--root", type=Path, default=None, help="Override benchmark base folder (defaults to get_data_path)")
